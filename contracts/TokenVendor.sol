...
contract TokenVendor is Ownable {
    ...
    //pricePerEth should change wih certain real world stuff
    ...
    function buyTokens(uint256 _amountToBuy) public payable {
        uint256 payment = _amountToBuy * pricePerEth;
        require(msg.value >= payment, "Insufficient Payment");

        if (s_myToken.balanceOf(address(this)) > _amountToBuy) {
            s_myToken.approve(address(this), _amountToBuy);
            s_myToken.transferFrom(address(this), msg.sender, _amountToBuy);
        } else {
            s_myToken.mint(msg.sender, _amountToBuy);
        }

        emit TokensBought(msg.sender, _amountToBuy);
    }
    ...
    function depositCollateral(uint256 _collateralAmount) public {
        ...
        s_addressToCollateral[msg.sender] = s_addressToCollateral[msg.sender].add(
            _collateralAmount
        );
        IERC20(wethAddress).safeTransferFrom(msg.sender, address(this), _collateralAmount);
        emit CollateralDeposited(msg.sender, _collateralAmount);
    }

    function withdrawCollateral(uint256 _amount) public WithdrawOnlyIfDeposited {
        require(_amount > 0, "Please enter a valid amount");
        require(s_addressToCollateral[msg.sender] >= _amount, "Insufficient Funds");
        IERC20(wethAddress).safeTransfer(msg.sender, _amount);
        s_addressToCollateral[msg.sender] = s_addressToCollateral[msg.sender].sub(_amount);
    ...
    function borrowMyToken(uint256 _amountToBorrow) public {
        ...
        require(_amountToBorrow <= borrowLimit, "You have reached the borrow limit");
        ...
        s_addressToBorrowedTokens[msg.sender] = s_addressToBorrowedTokens[msg.sender].add(
            _amountToBorrow
        );
    ...
    function repayMyToken(uint256 _repayAmount) public {
        ...
        require(borrowedTokens >= _repayAmount, "You haven't borrowed any tokens");
        s_addressToBorrowedTokens[msg.sender] = borrowedTokens.sub(_repayAmount);
    ...
    modifier WithdrawOnlyIfDeposited() {
        require(s_addressToCollateral[msg.sender] > 0, "No collateral deposited");
        _;
    }
